...
app.innerHTML = `
  <div class="shell">
    ...
    <aside class="sidepanel">
      ...
      <div><span>XP</span><strong id="xp">0 / 100</strong></div>
      <div><span>HP</span><strong id="hp">100 / 100</strong></div>
      <div><span>Kills</span><strong id="kills">0</strong></div>
      <div class="hint">WASD bewegen · Maus zielen · Linksklick feuern · Upgrade-Karte auswählen</div>
    </aside>
    <div id="upgradeOverlay" class="overlay hidden" aria-live="polite">
      <div class="upgradePanel"><div class="eyebrow">LEVEL UP</div><h2>Choose an upgrade</h2><div id="upgradeCards" class="upgradeCards"></div></div>
    </div>
  </div>`;
...
const upgradeCatalog: Record<string, { title: string; description: string; rarity: string }> = {
  heavy_bullets: { title: "Heavy Bullets", description: "+20% damage", rarity: "COMMON" },
  rapid_fire: { title: "Rapid Fire", description: "12% faster attacks", rarity: "COMMON" },
  twin_shot: { title: "Twin Shot", description: "+1 projectile", rarity: "UNCOMMON" },
  piercing: { title: "Piercing", description: "+1 pierce", rarity: "UNCOMMON" },
  velocity: { title: "Velocity", description: "+20% projectile speed", rarity: "COMMON" },
  adrenaline: { title: "Adrenaline", description: "+15% movement speed", rarity: "COMMON" },
  magnet: { title: "Magnet", description: "+45 pickup radius", rarity: "COMMON" },
  vitality: { title: "Vitality", description: "+25 max HP and heal", rarity: "COMMON" },
  armor: { title: "Armor Plating", description: "Reduce incoming damage", rarity: "UNCOMMON" },
  critical: { title: "Critical Strike", description: "+8% crit chance", rarity: "RARE" },
  deadeye: { title: "Deadeye", description: "+35% crit damage", rarity: "RARE" },
  overclock: { title: "Overclock", description: "18% faster attacks and +8% damage", rarity: "EPIC" },
};
...
class MainScene extends Phaser.Scene {
  ...
  private xpDrops = new Map<string, Phaser.GameObjects.Diamond>();
  ...
  sync(state: any) {
    this.syncMap(state.players, this.players, (id: string, p: any) => {
      const marker = this.add.circle(p.x, p.y, 18, id === localSessionId ? 0x67e8f9 : 0x8b5cf6);
      marker.setStrokeStyle(3, 0xffffff, 0.8);
      return marker;
    });
    this.syncMap(state.enemies, this.enemies, (_id: string, e: any) => {
      const colors: Record<string, number> = { basic: 0xf43f5e, fast: 0xfb923c, tank: 0x94a3b8, ranged: 0xa78bfa, elite: 0xfacc15, boss: 0xef4444 };
      const radius = e.radius ?? 16;
      const marker = this.add.circle(e.x, e.y, radius, colors[e.kind] ?? 0xf43f5e);
      marker.setStrokeStyle(e.boss ? 5 : e.elite ? 3 : 2, e.boss ? 0xfef2f2 : 0xffffff, 0.8);
      return marker;
    });
    this.syncMap(state.projectiles, this.projectiles, (_id: string, p: any) => this.add.circle(p.x, p.y, 5, 0xfef08a));
    this.syncMap(state.xpDrops, this.xpDrops, (_id: string, drop: any) => {
      const diamond = this.add.diamond(drop.x, drop.y, 11, 11, 0x22d3ee);
      diamond.setStrokeStyle(1, 0xcffafe, 0.8);
      return diamond;
    });
  }
  private syncMap<T extends Phaser.GameObjects.GameObject>(source: any, target: Map<string, T>, create: (id: string, data: any) => T) {
    const seen = new Set<string>();
    source.forEach((data: any, id: string) => {
      seen.add(id);
      let marker = target.get(id);
      if (!marker) { marker = create(id, data); target.set(id, marker); }
      if ("setPosition" in marker && typeof (marker as any).setPosition === "function") (marker as any).setPosition(data.x, data.y);
    });
    for (const [id, marker] of target) {
      if (!seen.has(id)) { marker.destroy(); target.delete(id); }
    }
  }
}
...
function renderUpgradeChoices(ids: string[]) {
  ...
  if (signature === lastUpgradeSignature || ids.length === 0) return;
  lastUpgradeSignature = signature;
  overlay.classList.remove("hidden");
  upgradeCards.innerHTML = "";
  ids.forEach((id, index) => {
    const info = upgradeCatalog[id] ?? { title: id, description: "Upgrade", rarity: "COMMON" };
    const button = document.createElement("button");
    button.className = "upgradeCard";
    button.type = "button";
    button.innerHTML = `<span class="rarity">${info.rarity}</span><strong>${index + 1}. ${info.title}</strong><span>${info.description}</span>`;
    button.addEventListener("click", () => { room?.send("chooseUpgrade", id); overlay.classList.add("hidden"); lastUpgradeSignature = ""; });
  ...
  try {
    ...
    room.onStateChange((state: any) => {
      ...
      if (local) {
        ...
        killsEl.textContent = String(local.kills);
        if (local.upgradeChoices && local.upgradeChoices !== "[]" && local.upgradeChoices !== "pending") {
          try { renderUpgradeChoices(JSON.parse(local.upgradeChoices) as string[]); } catch { overlay.classList.add("hidden"); }
        }
      }
