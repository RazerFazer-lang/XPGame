...
app.innerHTML = `
  <div class="shell"><header class="topbar"><div><div class="eyebrow">XPGame</div><h1>Horde Protocol</h1></div><div id="connection" class="status">OFFLINE</div></header>
  ...
  <div id="game" class="game"></div><aside class="sidepanel"><div><span>Phase</span><strong id="phase">LOBBY</strong></div><div><span>Wave</span><strong id="wave">0</strong></div><div><span>Players</span><strong id="players">0 / ${GAME.maxPlayers}</strong></div><div><span>Level</span><strong id="level">1</strong></div><div><span>XP</span><strong id="xp">0 / 100</strong></div><div><span>HP</span><strong id="hp">100 / 100</strong></div><div class="hint">WASD bewegen · Maus zielen · Linksklick feuern</div></aside></div>`;
...
class MainScene extends Phaser.Scene {
  ...
  sync(state: any) {
    ...
    this.syncMap(state.enemies, this.enemies, (_id: string, e: any) => { const marker = this.add.circle(e.x, e.y, e.radius ?? 16, 0xf43f5e); marker.setStrokeStyle(2, 0xffc0cb, 0.65); return marker; });
    this.syncMap(state.projectiles, this.projectiles, (_id: string, p: any) => this.add.circle(p.x, p.y, 5, 0xfef08a));
  }
  private syncMap(source: any, target: Map<string, Phaser.GameObjects.Arc>, create: (id: string, data: any) => Phaser.GameObjects.Arc) {
    const seen = new Set<string>();
    source.forEach((data: any, id: string) => { seen.add(id); let marker = target.get(id); if (!marker) { marker = create(id, data); target.set(id, marker); } marker.setPosition(data.x, data.y); });
...
async function connect() {
  ...
  try {
    ...
    localSessionId = room.sessionId; readyButton.disabled = false; connectionEl.textContent = "CONNECTED";
    room.onStateChange((state: any) => { phaseEl.textContent = String(state.phase).toUpperCase(); waveEl.textContent = String(state.wave); playersEl.textContent = `${state.players.size} / ${GAME.maxPlayers}`; const local = state.players.get(localSessionId); if (local) { levelEl.textContent = String(local.level); xpEl.textContent = `${Math.floor(local.xp)} / ${Math.floor(local.xpToNext)}`; hpEl.textContent = `${Math.ceil(local.hp)} / ${Math.ceil(local.maxHp)}`; } (game.scene.getScene("main") as MainScene).sync(state); });
