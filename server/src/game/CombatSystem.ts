...
export function createEnemy(wave: number, kind: EnemyKind = "basic", mapId = "neon_city"): EnemyState {
  const enemy = new EnemyState(); const angle = Math.random() * Math.PI * 2; const radius = 640;
  enemy.x = GAME.width / 2 + Math.cos(angle) * radius; enemy.y = GAME.height / 2 + Math.sin(angle) * radius;
  const map = MAPS[mapId as keyof typeof MAPS] ?? MAPS.neon_city;
  const scale = (1 + Math.max(0, wave - 1) * 0.14) * map.enemyHp; enemy.kind = kind; enemy.elite = kind === "elite" || kind === "miniboss"; enemy.boss = kind === "boss";
  const stats: Record<EnemyKind, { hp: number; speed: number; damage: number; xp: number; r: number }> = {
    basic: { hp: 30, speed: 70, damage: 8, xp: 10, r: 16 }, fast: { hp: 20, speed: 135, damage: 7, xp: 12, r: 13 }, tank: { hp: 120, speed: 42, damage: 15, xp: 24, r: 24 }, ranged: { hp: 48, speed: 55, damage: 10, xp: 18, r: 17 }, swarm: { hp: 14, speed: 165, damage: 5, xp: 8, r: 10 }, splitter: { hp: 70, speed: 64, damage: 12, xp: 28, r: 19 }, healer: { hp: 85, speed: 52, damage: 7, xp: 32, r: 19 }, elite: { hp: 240, speed: 82, damage: 20, xp: 65, r: 25 }, miniboss: { hp: 700, speed: 58, damage: 28, xp: 180, r: 34 }, boss: { hp: 1800, speed: 48, damage: 32, xp: 450, r: 48 },
  };
  const base = stats[kind]!; const eliteScale = enemy.boss ? 1 + Math.max(0, wave - 1) * 0.18 : enemy.elite ? 1 + Math.max(0, wave - 1) * 0.16 : 1;
  enemy.maxHp = base.hp * scale * eliteScale; enemy.hp = enemy.maxHp; enemy.speed = base.speed + Math.min(80, wave * 2); enemy.damage = base.damage * (1 + Math.max(0, wave - 1) * 0.06); enemy.radius = base.r; enemy.xp = Math.round(base.xp * (1 + Math.max(0, wave - 1) * 0.03)); enemy.phase = 1; return enemy;
}
...
export function moveEnemyTowardPlayer(enemy: EnemyState, player: PlayerState, deltaSeconds: number): void {
  ...
  if (enemy.kind === "ranged" && distance < 280) { enemy.x -= (dx / distance) * enemy.speed * deltaSeconds; enemy.y -= (dy / distance) * enemy.speed * deltaSeconds; return; }
  if (enemy.kind === "healer" && distance < 220) return;
  if (distance > enemy.radius + 20) { enemy.x += (dx / distance) * enemy.speed * deltaSeconds; enemy.y += (dy / distance) * enemy.speed * deltaSeconds; }
}
...
export function fireProjectile(state: GameState, playerId: string, nextId: () => string): boolean {
  ...
  for (let i = 0; i < count; i += 1) {
    const t = count === 1 ? 0 : i / (count - 1) - 0.5; const angle = Math.atan2(player.aimY, player.aimX) + t * totalSpread; const projectile = new ProjectileState();
    projectile.ownerId = playerId; projectile.x = player.x + Math.cos(angle) * 24; projectile.y = player.y + Math.sin(angle) * 24; projectile.vx = Math.cos(angle) * (player.projectileSpeed || weapon.speed); projectile.vy = Math.sin(angle) * (player.projectileSpeed || weapon.speed); projectile.damage = player.damage * weapon.multiplier * levelScale * (Math.random() < player.critChance ? player.critMultiplier : 1); projectile.pierce = Math.max(0, Math.floor(player.pierce)); state.projectiles.set(nextId(), projectile);
  }
  player.attackTimerMs = Math.max(70, Math.min(1600, Math.min(player.attackCooldownMs, weapon.cooldown))); return true;
}
